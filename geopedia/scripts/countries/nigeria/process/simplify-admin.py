"""
Simplify Nigeria's canonical administrative GeoJSON for runtime use.

The canonical intermediate files remain unchanged. Simplified copies are
written to the public runtime data directory for use by GeoPedia's maps.
...
Outputs:
    public/data/countries/nigeria/geojson/states.geojson
    public/data/countries/nigeria/geojson/local-government-areas.geojson
"""
...
DATASETS = [
    ...
    {
        "name": "LGAs",
        "filename": "local-government-areas.geojson",
        "tolerance": 0.0015,
        "expected_count": 774,
    },
]
...
def simplify_feature(
    ...
    """
    Simplify one feature while preserving topology within its own geometry.

    Properties are copied unchanged so map identity, answers, and grouping
    continue to use the canonical administrative values.
    """
...
def main() -> None:
    """Create Nigeria's simplified runtime administrative GeoJSON files."""
    print("Simplifying Nigeria administrative boundaries...")
    print()

    for dataset in DATASETS:
        process_dataset(dataset)

    print("Nigeria administrative simplification complete.")
