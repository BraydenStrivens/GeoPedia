"""Process Dominican Republic provinces for GeoPedia.

Reads the raw ADM2 administrative-boundary GeoJSON sourced from the Dominican
Republic Oficina Nacional de Estadística (ONE) through OCHA/HDX and produces
a simplified runtime GeoJSON containing the country's 32 province-level units.

Processing:
- Validates the expected source schema and feature count.
- Normalizes ONE pcodes such as DO0101 to province IDs such as 0101.
- Normalizes parent ADM1 pcodes such as DO01 to region IDs such as 01.
- Removes the "Provincia " administrative prefix from province names.
- Preserves "Distrito Nacional" as-is.
- Validates ADM2 -> ADM1 hierarchy consistency.
...
def load_source() -> gpd.GeoDataFrame:
    """Load and validate the raw ADM2 source dataset."""
    ...
    if gdf["adm2_pcode"].duplicated().any():
        duplicates = sorted(
            gdf.loc[gdf["adm2_pcode"].duplicated(False), "adm2_pcode"]
            .astype(str)
            .unique()
        )
        raise ValueError(f"Duplicate ADM2 pcodes found: {duplicates}")
...
def normalize_province_name(name: str) -> str:
    """Remove the province prefix while preserving Distrito Nacional."""
    name = name.strip()

    if name == "Distrito Nacional":
        return name
    ...
    if not name.startswith(prefix):
        raise ValueError(
            "ADM2 name is neither a province nor Distrito Nacional: "
            f"{name!r}"
        )
    ...
    if not display_name:
        raise ValueError(
            f"ADM2 name is empty after removing its prefix: {name!r}"
        )
...
def validate_source_hierarchy(gdf: gpd.GeoDataFrame) -> None:
    """Validate that each ADM2 pcode agrees with its ADM1 parent pcode."""
    for _, row in gdf.iterrows():
        province_id = normalize_province_id(str(row["adm2_pcode"]))
        region_id = normalize_region_id(str(row["adm1_pcode"]))

        if province_id[:2] != region_id:
            raise ValueError(
                "ADM2 hierarchy mismatch: "
                f"{row['adm2_pcode']} belongs to {row['adm1_pcode']}, "
                f"but normalized IDs are {province_id!r} and {region_id!r}."
            )

        if region_id not in EXPECTED_REGION_IDS:
            raise ValueError(
                f"Unexpected ADM1 region ID on ADM2 feature: {region_id!r}"
            )
    ...
    for _, row in gdf.iterrows():
        ...
        feature = {
            ...
            "properties": {
                ...
                "name": normalize_province_name(str(row["adm2_name"])),
                "region_id": region_id,
            },
...
def validate_output(features: list[dict[str, Any]]) -> None:
    ...
    if len(set(province_ids)) != EXPECTED_FEATURE_COUNT:
        raise ValueError("Output province IDs are not unique.")

    if len(set(names)) != EXPECTED_FEATURE_COUNT:
        raise ValueError("Output province names are not unique.")
...
def main() -> None:
    """Process the Dominican Republic ADM2 province-level units."""
    print("Processing Dominican Republic provinces")
    ...
    print()
    print("Output provinces:")

    for feature in features:
        properties = feature["properties"]
        print(
            f"  {properties['province_id']}: "
            f"{properties['name']} "
