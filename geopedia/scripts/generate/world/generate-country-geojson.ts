...
function generateCountryGeoJson(): void {
  ...
  // TEST ------------------------------------------------------------------------
  for (const [countryId, geometryData] of geometryLookup) {
    if (geometryData.geometries.length > 1) {
      console.log(
        `${countryId} merged from ${geometryData.geometries.length} Natural Earth map units.`,
      );
    }
  }
  // TEST ------------------------------------------------------------------------
