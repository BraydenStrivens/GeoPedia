 ...
 *
 * Shared country-name metadata supplies each country's English and optional
 * native/local name. REST Countries supplies official names and other general
 * country metadata.
 *
 ...
 *
 * English and native/local names come from GeoPedia's shared country-name
 * metadata so country pages use the same names as global country quizzes.
 * REST Countries continues to supply the official name and other general
 * metadata.
 */
function createGeneratedCountry(
  ...
  const countryNames = COUNTRY_NAMES[countryId.toUpperCase()];

  if (!countryNames) {
    throw new Error(
      `Country ${countryId} has no shared country-name metadata.`,
    );
  }
  ...
  return {
    ...
    nativeName: countryNames.nativeName,
    officialName,
    continent,
    region,
...
function createCountryFile(country: GeneratedCountry): string {
  const nativeNameLine =
    country.nativeName === undefined
      ? ""
      : `  nativeName: ${tsString(country.nativeName)},\n`;
...
${nativeNameLine}  officialName: ${tsString(country.officialName)},
...
function main(): void {
  ...
  const nativeNameCount = generatedCountries.filter(
  ...
  console.log(
    `CountryData objects with native names: ${nativeNameCount}`,
  );
  console.log(
